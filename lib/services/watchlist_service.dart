...
class WatchlistService {
  ...
  static final List<WatchlistStock> _dummyStocks = [
    WatchlistStock(
      ...
      keyword: 'HBM',
      aiSummary:
          'HBM 수요 증가로 반도체 실적 개선 기대감이 높아지고 있습니다.\n'
    ...
    WatchlistStock(
      ...
      keyword: 'AI반도체',
      aiSummary:
          'AI 서버용 HBM3E 공급 확대로 분기 실적이 크게 개선되었습니다.\n'
    ...
    WatchlistStock(
      ...
      keyword: '검색광고',
      aiSummary:
          '검색 광고 매출은 안정적이나 성장률 둔화 우려가 있습니다.\n'
    ...
    WatchlistStock(
      ...
      keyword: '전고체배터리',
      aiSummary:
          '전고체 배터리 양산 로드맵 발표로 기술력이 부각되었습니다.\n'
  ...
  static final WatchlistBriefing _dummyBriefing = WatchlistBriefing(
    text:
        '오늘 관심종목 중 반도체 섹터가 강세를 보이고 있습니다. '
  ...
  Future<List<WatchlistStock>> getWatchlist() async {
    ...
    try {
      final res = await http
          .get(Uri.parse('$base/api/watchlist'))
  ...
  Future<bool> deleteStock(String code) async {
    ...
    try {
      final res = await http
          .delete(Uri.parse('$base/api/watchlist/$stockCode'))
          .timeout(const Duration(seconds: 10));
  ...
  Future<WatchlistBriefing> getBriefing() async {
    ...
    try {
      final res = await http
          .get(Uri.parse('$base/api/watchlist/briefing'))
          .timeout(const Duration(seconds: 10));
      if (res.statusCode == 200) {
  ...
  Future<WatchlistStock?> getStockDetail(String code) async {
    ...
    try {
      final res = await http
          .get(Uri.parse('$base/api/stocks/$code'))
          .timeout(const Duration(seconds: 10));
      if (res.statusCode == 200) {
